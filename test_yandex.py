...
def test_yandex_connection():
    ...
    try:
        ...
        print(f"✅ Подключение к Яндекс.Музыка API успешно!")
        ...
        if playlists:
            ...
            try:
                playlist_tracks = client.users_playlists(playlists[0].kind, user_id=user.uid).fetch_tracks()
                print(f"✅ Получено треков из плейлиста: {len(playlist_tracks)}")
